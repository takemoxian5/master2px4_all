...
static u8 StopCondition=0; 								//?????????????¦Ë,DMA????§Õ????????  
volatile SD_Error TransferError=SD_OK;					//????????????,DMA??§Õ????	    
volatile u8 TransferEnd=0;								//??????????,DMA??§Õ????
...
SD_Error SD_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_SDIO, ENABLE);//SDIO??????
	...
	RCC_APB2PeriphResetCmd(RCC_APB2Periph_SDIO, ENABLE);//SDIO??¦Ë
  ...
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource2,GPIO_AF_SDIO);	
	...
	RCC_APB2PeriphResetCmd(RCC_APB2Periph_SDIO, DISABLE);//SDIO??????¦Ë
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
	...
	NVIC_Init(&NVIC_InitStructure);	//?????????????????VIC???????
 	...
 	if(errorstatus==SD_OK)errorstatus=SD_SelectDeselect((u32)(SDCardInfo.RCA<<16));//???SD??   
   	if(errorstatus==SD_OK)errorstatus=SD_EnableWideBusOperation(SDIO_BusWide_4b);	//4¦Ë???,?????MMC??,??????4¦Ë?? 
...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;  //?????bypass?????????HCLK???§Ù?????SDIO_CK
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;	// ???????????????
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;	 				//1¦Ë??????
  ...
  SDIO->CLKCR|=1<<8;			//SDIOCK???  
    ...
    SDIO_SendCommand(&SDIO_CmdInitStructure);	  		//§Õ?????????????
  ...
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;			 //??????§Ø?
	...
	{																  
      ...
      SDIO_SendCommand(&SDIO_CmdInitStructure);
			...
			errorstatus=CmdResp3Error(); 					//???R3???   
 			...
 			if(errorstatus!=SD_OK)return errorstatus;   	//??????? 
			response=SDIO->RESP1;;			   				//??????
			validvoltage=(((response>>31)==1)?1:0);			//?§Ø?SD???????????
			count++;
		}
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF)	//???????,?????????
  {
	...
	if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_SECURE_DIGITAL_IO_COMBO_CARD==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))//?§Ø??????
	{
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	cardinfo->SD_csd.SysSpecVersion=(tmp&0x3C)>>2;	//2.0§¿?íé?????????(?????),????????§¿?úE???
	cardinfo->SD_csd.Reserved1=tmp&0x03;			//2??????¦Ë  
	...
	cardinfo->SD_csd.CardComdClasses=tmp<<4;    	//??????????¦Ë
	tmp=(u8)((CSD_Tab[1]&0x00FF0000)>>16);	 		//??5?????
	cardinfo->SD_csd.CardComdClasses|=(tmp&0xF0)>>4;//??????????¦Ë
	cardinfo->SD_csd.RdBlockLen=tmp&0x0F;	    	//????????????
	tmp=(u8)((CSD_Tab[1]&0x0000FF00)>>8);			//??6?????
	cardinfo->SD_csd.PartBlockRead=(tmp&0x80)>>7;	//???????
	cardinfo->SD_csd.WrBlockMisalign=(tmp&0x40)>>6;	//§Õ???¦Ë
	cardinfo->SD_csd.RdBlockMisalign=(tmp&0x20)>>5;	//?????¦Ë
	...
	{
		cardinfo->SD_csd.DeviceSize=(tmp&0x03)<<10;	//C_SIZE(12¦Ë)
...
{	  
  ...
  SDIO->DCTRL=0x0;	//???????????????(??DMA) 
	...
	{
 		INTX_DISABLE();//??????§Ø?(POLLING??,????§Ø???SDIO??§Õ????!!!)
		while(!(SDIO->STA&((1<<5)|(1<<1)|(1<<3)|(1<<10)|(1<<9))))//??????/CRC/???/???(???)/???¦Ë????
		{
		...
		}else if(SDIO_GetFlagStatus(SDIO_FLAG_STBITERR) != RESET) 	//???????¦Ë????
		{
		...
		INTX_ENABLE();//???????§Ø?
		SDIO_ClearFlag(SDIO_STATIC_FLAGS);//??????§Ò??
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		StopCondition=0;			//?????,??????????????????
		TransferEnd=0;				//???????????¦Ë?????§Ø??????1
		SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<5)|(1<<9);	//??????????§Ø? 
...
SD_Error SD_ReadMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
  ...
  SDIO->DCTRL=0x0;		//???????????????(??DMA)   
	...
	if(nblks>1)											//????  
	{									    
 	  	if(nblks*blksize>SD_MAX_DATA_LENGTH)return SD_INVALID_PARAMETER;//?§Ø??????????????? 
       ...
       SDIO_CmdInitStructure.SDIO_Argument =  addr;//????CMD18+??addr????????????,????? 
		...
		{
			INTX_DISABLE();//??????§Ø?(POLLING??,????§Ø???SDIO??§Õ????!!!)
			while(!(SDIO->STA&((1<<5)|(1<<1)|(1<<3)|(1<<8)|(1<<9))))//??????/CRC/???/???(???)/???¦Ë????
			{
				if(SDIO_GetFlagStatus(SDIO_FLAG_RXFIFOHF) != RESET)						//??????????,??????????8????
				{
					for(count=0;count<8;count++)			//??????????
...
SD_Error SD_WriteBlock(u8 *buf,long long addr,  u16 blksize)
{
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	timeout=SD_DATATIMEOUT;
   	while(((cardstatus&0x00000100)==0)&&(timeout>0)) 	//???READY_FOR_DATA¦Ë?????¦Ë
	{
		timeout--;  
	...
	if(timeout==0)return SD_ERROR;
			...
			SDIO_CmdInitStructure.SDIO_Argument = addr;//????CMD24,§Õ???????,????? 	
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	{
		INTX_DISABLE();//??????§Ø?(POLLING??,????§Ø???SDIO??§Õ????!!!)
		while(!(SDIO->STA&((1<<10)|(1<<4)|(1<<1)|(1<<3)|(1<<9))))//????ú€????/????/CRC/???/???¦Ë????
		{
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
		...
		StopCondition=0;			//????§Õ,?????????????????? 
		TransferEnd=0;				//???????????¦Ë?????§Ø??????1
		SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<4)|(1<<9);	//???¨°??????????????§Ø?
	...
	{					  
     	...
     	if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))
    	{
			...
			if(errorstatus!=SD_OK)return errorstatus;		
				...
				SDIO_CmdInitStructure.SDIO_Argument =addr;	//????CMD25,???§Õ???,????? 	  
		...
		if(DeviceMode==SD_POLLING_MODE)
	    {
			timeout=SDIO_DATATIMEOUT;
			INTX_DISABLE();//??????§Ø?(POLLING??,????§Ø???SDIO??§Õ????!!!)
			while(!(SDIO->STA&((1<<4)|(1<<1)|(1<<8)|(1<<3)|(1<<9))))//????/CRC/???????/???/???¦Ë????
			{
...
{
	if(SDIO_GetFlagStatus(SDIO_FLAG_DATAEND) != RESET)//????????§Ø?
	{	 
		if (StopCondition==1)
		{  
				SDIO_CmdInitStructure.SDIO_Argument =0;//????CMD12+???????? 	  
 		...
 		SDIO->ICR|=1<<8;//???????§Ø???
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//???????§Ø?
 		TransferEnd = 1;
 	...
 	if(SDIO_GetFlagStatus(SDIO_FLAG_DCRCFAIL) != RESET)//????CRC????
	{
 	...
 	if(SDIO_GetFlagStatus(SDIO_FLAG_DTIMEOUT) != RESET)//??????????
	{
		SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT);  			//???§Ø???
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//???????§Ø?
	...
	{
		SDIO_ClearFlag(SDIO_FLAG_RXOVERR);  			//???§Ø???
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//???????§Ø?
	...
	{
		SDIO_ClearFlag(SDIO_FLAG_TXUNDERR);  			//???§Ø???
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//???????§Ø?
	    TransferError = SD_TX_UNDERRUN;
	    return(SD_TX_UNDERRUN);
	}
	if(SDIO_GetFlagStatus(SDIO_FLAG_STBITERR) != RESET)//???¦Ë????
	{
		SDIO_ClearFlag(SDIO_FLAG_STBITERR);  		//???§Ø???
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//???????§Ø?
...
SD_Error CmdResp7Error(void)
{
	...
	{
		status=SDIO->STA;
		if(status&((1<<0)|(1<<2)|(1<<6)))break;//CRC????/??????????/?????????(CRC§µ????)	
	}
 	if((timeout==0)||(status&(1<<2)))	//??????
	{																				    
		errorstatus=SD_CMD_RSP_TIMEOUT;	//?????????2.0?????,?????????Ú…??????¦¶
	...
	if((scr[1]&SD_WIDE_BUS_SUPPORT)!=SD_ALLZERO)		//????????
	{
      ...
      SDIO_SendCommand(&SDIO_CmdInitStructure);
	 	...
	 	if(errorstatus!=SD_OK)return errorstatus; 
		  ...
		  SDIO_CmdInitStructure.SDIO_Argument = arg;//????ACMD6,?????,????:10,4¦Ë;00,1¦Ë.	
...
//mbuf:?›¥?????
//bufsize:??????????
//dir:????;DMA_DIR_MemoryToPeripheral  ?›¥??-->SDIO(§Õ????);DMA_DIR_PeripheralToMemory SDIO-->?›¥??(??????);
...
{		 
  ...
  DMA_DeInit(DMA2_Stream3);//???????stream3????????§Ø???
  ...
  DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_INC4;//???????4?¦Ä???
  DMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_INC4;//?›¥?????4?¦Ä???
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
			memcpy(SDIO_DATA_BUFFER,buf,512);
		 	sta=SD_WriteBlock(SDIO_DATA_BUFFER,lsector+512*n,512);//????sector??§Õ????
			buf+=512;
		} 
	}else
	{
		if(cnt==1)sta=SD_WriteBlock(buf,lsector,512);    	//????sector??§Õ????
